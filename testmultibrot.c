...
/*   Updated: 2023/05/22 15:59:45 by okraus           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// x=xtmp
	...
	{
		max->data->temp = powl((zx * zx) + (zy * zy), (max->data->power / 2)) * cosl(max->data->power * atan2l(zy, zx)) + cx;
		zy = powl(((zx * zx) + (zy * zy)), (max->data->power / 2)) * sinl(max->data->power * atan2l(zy, zx)) + cy;
		zx = max->data->temp;
		iter++;
	}
	return (iter);
